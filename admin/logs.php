...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($action) || !in_array($action, ['archive', 'restore'])) {
        // If action is not specified in the JSON body, or is invalid, stop processing.
    ...
    try {
        if ($action === 'archive') {
            ...
            $delAccount  = $pdo->prepare("DELETE FROM account_logs WHERE action_id = ?");

            foreach ($input['logs'] as $log) {
                // 1. Fetch original data to ensure we capture it correctly
                $table = ($log['log_source'] === 'Schedule Log') ? 'schedule_logs' : 'account_logs';
                $stmt = $pdo->prepare("SELECT * FROM $table WHERE action_id = ?");
                $stmt->execute([$log['action_id']]);
                $record = $stmt->fetch(PDO::FETCH_ASSOC);

                if ($record) {
                    // 2. Insert into archived_logs
                    ...
                    if ($log['log_source'] === 'Schedule Log') {
                        $delSchedule->execute([$log['action_id']]);
                    } else {
                        $delAccount->execute([$log['action_id']]);
                    }
                }
            }
            ...
            $insertAccount  = $pdo->prepare("INSERT INTO account_logs (action_id, action_type, action_details, performed_by, timestamp) VALUES (?, ?, ?, ?, ?)");
            ...
            foreach ($input['logs'] as $log) {
                ...
                $stmt->execute([$log['action_id'], $log['log_source']]);
                $record = $stmt->fetch(PDO::FETCH_ASSOC);
                ...
                if ($record) {
                    ...
                    $delArchive->execute([$log['action_id'], $log['log_source']]);
                }
            }
...
if ($view_mode === 'archived') {
    ...
    $base_query = "
        ...
        FROM archived_logs
    ";
    $count_sql = "SELECT COUNT(*) FROM archived_logs $where_sql";
    $data_sql  = "$base_query $where_sql ORDER BY timestamp DESC LIMIT ? OFFSET ?";

} else {
    // Default: Active Logs (UNION of schedule and account logs)
    $base_query = "
    SELECT * FROM (
        SELECT 
            action_id COLLATE utf8mb4_unicode_ci AS action_id,
        ...
        SELECT 
            ...
            performed_by COLLATE utf8mb4_unicode_ci AS performed_by,
            timestamp,
            'Account Log' AS log_source
        FROM account_logs
    ) AS combined_logs
    ";
    $count_sql = "SELECT COUNT(*) FROM ($base_query) AS counted_tbl $where_sql";
    $data_sql  = "$base_query $where_sql ORDER BY timestamp DESC LIMIT ? OFFSET ?";
}
...
// Fetch unique action types for filter (cache this if possible in production)
$types1 = $pdo->query("SELECT DISTINCT action_type FROM schedule_logs")->fetchAll(PDO::FETCH_COLUMN);
$types2 = $pdo->query("SELECT DISTINCT action_type FROM account_logs")->fetchAll(PDO::FETCH_COLUMN);
$actionTypes = array_unique(array_merge($types1, $types2));
sort($actionTypes);
...
<head>
  ...
  <style>
    ...
    h2 { margin-bottom: 20px; display: flex; align-items: center; justify-content: space-between; }
    ...
    .filters input, .filters select, .filters button, .filters a { padding: 8px 10px; fontSize: 14px; border-radius: 6px; border: 1px solid #ccc; text-decoration: none; flex-shrink: 0; }
    ...
    /* Archive specific styling for rows if needed */
    .archived-row { background-color: #fff8e1; }
    ...
    /* Pagination Styles */
    .pagination-container { display: flex; align-items: center; gap: 10px; flex-shrink: 0; }
    ...
    .pagination-controls-bottom { margin-top: 20px; }
...
<body>
  <div class="content">
    <div class="logs-container">
      <h2>
        <span><i class="fa-solid fa-history"></i>‎ ‎ System Logs</span>
      </h2>
      ...
      <div id="table-container">
          ...
          <p id="no-logs-message" style="<?= count($logs) > 0 ? 'display:none;' : 'display:block;' ?>">No logs found matching your criteria.</p>
      </div>

      <div class="pagination-controls-bottom" style="display: flex; justify-content: space-between; align-items: center; margin-top: 20px;">
        <p class="page-info">
            Showing <?= count($logs) > 0 ? $offset + 1 : 0; ?> to <?= $offset + count($logs) ?> of <?= $total_rows ?> entries
        </p>
        ...
        <ul class="pagination">
            <?php
            // Reuse previous logic for bottom pagination
            if ($current_page > 1) {
                $query_params['page'] = $current_page - 1;
                echo '<li><a href="?' . http_build_query($query_params) . '" data-page="' . ($current_page - 1) . '">&laquo;</a></li>';
            } else {
...
function updateButtons(viewMode) {
    // We need to re-attach or toggle the visibility of the bulk buttons based on viewMode
    // However, since the filter form is static HTML, we might need to swap the button manually using JS if we want pure client-side feel,
    // OR simply rely on the fetch logic.
    // In this implementation, the backend renders the buttons on page load, but AJAX updates might miss this.
    // Let's force update the specific container:
    ...
    if(viewMode === 'active') {
        ...
        if(!existingRestore) {
            ...
            btn.addEventListener("click", handleRestoreSelected); // Re-bind listener
            const delBtn = btnContainer.querySelector('.delete-selected');
            btnContainer.insertBefore(btn, delBtn);
        }
    }
...
if(document.getElementById("deleteSelectedBtn")) {
    document.getElementById("deleteSelectedBtn").addEventListener("click", function() {
        ...
        if (confirm(`Permanently delete ${selectedLogs.length} selected log(s)?`)) {
...
// Handlers for Archive/Restore buttons (to be re-attached dynamically)
...
// Function to handle Archive AND Restore (handled by logs.php logic at top)
...
// Use original delete functionality logic via delete_logs.php
...
function exportToExcel() {
  ...
  XLSX.writeFile(wb, `logs_export_${now}.xlsx`);
}
...
function exportToCSV() {
  ...
  link.download = `logs_export_${now}.csv`;
  link.click();
}
...
function downloadBackup() {
  ...
  const now = new Date().toISOString().replace(/[-:T]/g,"_").split(".")[0];
  link.href = URL.createObjectURL(blob);
  link.download = `logs_backup_${now}.json`;
  link.click();
}
